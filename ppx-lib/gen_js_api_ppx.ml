...
let print_error ppf = function
  ...
  | Multiple_inputs -> Format.fprintf ppf "A single input file must be provided"
  | Unlabelled_argument_in_builder ->
...
type class_field = Method of method_decl | Inherit of Longident.t Location.loc
  ...
  | Ptyp_arrow (lab, t1, t2) when has_attribute "js.variadic" t1.ptyp_attributes
    -> (
      ...
      | { lab; att; typ = Name ("list", [ typ ]) } -> (
          ...
          | tres ->
              Arrow { ty_args = []; ty_vararg; unit_arg = false; ty_res = tres }
          )
...
let parse_attr ~global_attrs ?ty (s, loc, auto) attribute =
  ...
  let actions =
    [
      ...
      ("js", fun () -> auto ());
      ( "js.new",
        fun () -> New (opt_name ~prefix:"new_" ~global:true ~capitalize:true ())
      );
...
and parse_class_decl ~global_attrs = function
  | {
            ...
            Pcty_arrow
              ( Nolabel,
                ...
                },
                {
                  pcty_desc =
                    Pcty_signature
                      { pcsig_self = { ptyp_desc = Ptyp_any; _ }; pcsig_fields };
                  pcty_loc = _;
                  pcty_attributes = _;
                } );
          _;
      ...
      Declaration
        {
          class_name;
          class_fields = List.map (parse_class_field ~global_attrs) pcsig_fields;
        }
  | {
      pci_virt = Concrete;
      pci_params = [];
      ...
      let class_arrow =
        ...
        | Arrow
            ( { ty_args = _; ty_vararg = _; unit_arg = _; ty_res = Name (_, []) }
            as params ) ->
            params
        | Name (_, []) as ty_res ->
...
and parse_class_field ~global_attrs = function
  | {
      ...
      in
      Method { method_name; method_typ = ty; method_def; method_loc = pctf_loc }
...
and ml2js ty exp =
  ...
  | Arrow { ty_args; ty_vararg = None; unit_arg; ty_res } ->
      ...
      let body =
        if formal_args = [] then Exp.fun_ Nolabel None (Pat.any ()) res else res
      in
  ...
  | Arrow
      ...
      let concrete_args =
        List.mapi
          (fun i { lab; att = _; typ } ->
            (arg_label lab, js2ml typ (ojs "array_get" [ var arguments; int i ])))
          ty_args
      in
      let extra_arg =
        ojs "list_of_js_from"
  ...
  if
    ...
    && List.for_all
         (function
           | {
               typ =
                 Variant { location = _; global_attrs = _; attributes; constrs };
               _;
             }
...
let standalone () =
  ...
  let sg =
    Pparse.parse_interface ~tool_name:"gen_js_iface" src
    |> from_current.Migrate_parsetree.Versions.copy_signature
  in
  ...
  Format.fprintf
    ...
    "%a@." Pprintast.structure
    (to_current.copy_structure str);
...
let mark_attributes_as_used mapper =
  ...
  let attribute : _ -> attribute -> _ =
   fun this ({ attr_name = { txt; _ }; _ } as attr) ->
    if is_js_attribute txt then ignore (filter_attr txt attr : bool);
