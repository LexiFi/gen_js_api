...
include (
  [%js] :
    sig
      ...
      val caller : (unit -> int) -> int [@@js.global "caller"]

      val caller_unit : (unit -> unit) -> unit [@@js.global "caller"]
...
let () =
  ...
  test_variadic sum;
  test_variadic2 (fun msg xs ->
      Printf.printf "%s\n%!" msg;
      sum xs)

val myArray : int array [@@js]

val myArray2 : Ojs.t [@@js.global "myArray"]

val alert_bool : bool -> unit [@@js.global "alert"]

val alert_float : float -> unit [@@js.global "alert"]
...
let button ?attrs s onclick = elt "button" ?attrs ~onclick [ txt s ]
...
*)
  ...
  in
  let string_of_name_foo name foo =
    Printf.sprintf "%s <%s>" name (string_of_foo foo)
  in
  ...
  Element.appendChild body
    (div
       (List.map
          (fun x -> txt (string_of_person_obj x))
          [ alice_obj; bob_obj; dave_obj ]));
  ...
  Console.log_string console
    (Date.to_string (Date.create ~year:2015 ~month:4 ~day:10 ()));
  ...
  test_opt_args (fun ?(foo = 0) ?(bar = 0) () ->
      string_of_int foo ^ "/" ^ string_of_int bar);

  print_endline
    Person2.(to_json (mk ~children:[ mk ~age:6 "Johnny" ] ~age:42 "John Doe"))
...
module Dict : sig
  ...
  let t_of_js js2ml o =
    let l = ref [] in
    Ojs.iter_properties o (fun k -> l := (k, js2ml (Ojs.get o k)) :: !l);
    !l
end
...
val myDict : string Dict.t [@@js.global "myDict"]

val set_x : int -> unit [@@js.set "x"]

val get_x : unit -> int [@@js.get "x"]
...
module Sum = struct
  ...
  let print = function
    ...
    | D { age; name } ->
        print_endline (Format.sprintf "D {age = %d; name = %S}" age name)

  val set_print_sum : (t -> unit) -> unit [@@js.set "print_sum"]

  val test_sum : unit -> unit [@@js.global "test_sum"]

  let () = set_print_sum print
...
val test_flatten :
  ([ `A | `B of int | `C of string | `D of int * string ][@js.enum]) -> unit
