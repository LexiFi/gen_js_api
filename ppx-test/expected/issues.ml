...
module Issue109 : sig type t = [ `S of string  | `I of int ] end =
  ((struct
      ...
      let rec (t_of_js : Ojs.t -> t) =
        fun x23 ->
          let x24 = x23 in
          match Ojs.type_of x24 with
          | "number" -> (match Ojs.int_of_js x24 with | x26 -> `I x26)
          | "string" -> (match Ojs.string_of_js x24 with | x25 -> `S x25)
          | _ -> assert false
      and (t_to_js : t -> Ojs.t) =
        fun x20 ->
          match x20 with
          | `S x21 -> Ojs.string_to_js x21
          | `I x22 -> Ojs.int_to_js x22
    end)[@merlin.hide ]) 
